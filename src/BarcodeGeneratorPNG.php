...
class BarcodeGeneratorPNG extends BarcodeGenerator
{
    ...
    public function getBarcode($code, $type, $widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0))
    {
        ...
        $height = $totalHeight + 20;//20像素为下方文字高度
        ...
        //draw text
        $length = strlen($code);
        imagefilledrectangle($png, 0, $totalHeight, $width, $height, null);

        $color1 = imagecolorallocate($png, 0, 0, 0);

        $xOffset = ($width - 14 * $length) / 2;
        for ($i = 0; $i < $length; $i++) {
            $x = $xOffset + $i * 14;
            $y = 63;
            imagestring($png, 14, $x, $y, $code[$i], $color1);
        }
